    ...
    <article className="panel evidence-card">
      ...
      <div className="star-grid">
        {starFields.map((field) => (
          <label key={field}>
            {field}
            <textarea rows="4" value={draft[field] || ''} onChange={(event) => updateField(field, event.target.value)} />
          </label>
        ))}
      </div>
      ...
      {(draft.ksbs.length > 0 || draft.acceptanceCriteria.length > 0) && (
        <section className="suggestions">
          ...
          <SuggestionGroup title="KSBs" type="ksb" evidence={draft} items={draft.ksbs} saving={saving} onReview={onReview} />
          <SuggestionGroup title="Acceptance criteria" type="ac" evidence={draft} items={draft.acceptanceCriteria} saving={saving} onReview={onReview} />
        </section>
      )}
    </article>
  )
