...
const catalogue = {
  ...
  acceptanceCriteria: [
    { code: 'AC01', description: 'Describe the lifecycle.', required_ksb_codes: ['K1'] },
  ],
}
...
test('generateEvidenceFromNotes sends catalogue context and returns validated structured output', async () => {
                ...
                ksbSuggestions: [
                  {
                    code: 'K1',
                    confidence: 0.9,
                    rationale: 'The evidence discusses lifecycle stages.',
                  },
                ],
                acSuggestions: [
                  { code: 'AC01', confidence: 0.8, rationale: 'The notes describe the lifecycle.' },
                ],
              }),
            },
          },
  ...
  const result = await generateEvidenceFromNotes({
    rawNotes: 'Explained SDLC stages',
    ...catalogue,
  })

  assert.equal(requestBody.response_format.type, 'json_object')
  assert.match(requestBody.messages[0].content, /Explained SDLC stages/)
  assert.deepEqual(result.ksbSuggestions[0], {
    code: 'K1',
    confidence: 0.9,
    rationale: 'The evidence discusses lifecycle stages.',
  })
  assert.deepEqual(result.acSuggestions[0], {
    code: 'AC01',
    confidence: 0.8,
    rationale: 'The notes describe the lifecycle.',
  })
})
