...
export async function getKsbDetails(id) {
  return Promise.all([
    pool.query("SELECT ac.id, ac.code, ac.level, ac.description FROM acceptance_criteria ac JOIN acceptance_criteria_ksbs ack ON ack.acceptance_criteria_id = ac.id WHERE ack.ksb_id = $1 ORDER BY ac.code", [id]),
    pool.query("SELECT e.id, e.task_id, e.title, e.status, ek.review_status, ek.ai_confidence FROM evidence e JOIN evidence_ksbs ek ON ek.evidence_id = e.id WHERE ek.ksb_id = $1 ORDER BY e.created_at DESC", [id]),
  ]);
}
...
export async function getAcceptanceCriterionDetails(id) {
  return Promise.all([
    pool.query("SELECT k.id, k.code, k.type, k.description FROM ksbs k JOIN acceptance_criteria_ksbs ack ON ack.ksb_id = k.id WHERE ack.acceptance_criteria_id = $1 ORDER BY k.code", [id]),
    pool.query("SELECT e.id, e.task_id, e.title, e.status, eac.review_status, eac.ai_confidence, eac.ai_suggests_complete, eac.ai_completion_rationale FROM evidence e JOIN evidence_acceptance_criteria eac ON eac.evidence_id = e.id WHERE eac.acceptance_criteria_id = $1 ORDER BY e.created_at DESC", [id]),
  ]);
}

export function getAcceptanceCriterionEvidenceByCode(code) {
  return Promise.all([
    pool.query("SELECT id, code, level, description, keywords FROM acceptance_criteria WHERE code = $1", [code]),
    pool.query("SELECT k.code FROM ksbs k JOIN acceptance_criteria_ksbs ack ON ack.ksb_id = k.id JOIN acceptance_criteria ac ON ac.id = ack.acceptance_criteria_id WHERE ac.code = $1 ORDER BY k.code", [code]),
    pool.query(`
