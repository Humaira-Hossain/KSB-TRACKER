...
export function createNote({ title, content }) {
  return pool.query("INSERT INTO notes (title, content) VALUES ($1, $2) RETURNING *", [title, content]);
}
...
export function updateNote(id, updates) {
  ...
  values.push(id);
  return pool.query(`UPDATE notes SET ${assignments.join(", ")} WHERE id = $${values.length} RETURNING *`, values);
}
