...
export function listTasks() {
  return pool.query(
    'SELECT t.*, COUNT(e.id)::int AS evidence_count FROM tasks t LEFT JOIN evidence e ON e.task_id = t.id GROUP BY t.id ORDER BY t.created_at DESC',
  )
}
...
export function createTask({ title, rawNotes, status }) {
  return pool.query(
    "INSERT INTO tasks (title, raw_notes, status, completed_at) VALUES ($1, $2, $3::varchar, CASE WHEN $3::varchar = 'completed' THEN NOW() ELSE NULL END) RETURNING *",
    [title, rawNotes, status],
  )
}
...
export function updateTask(id, updates) {
  ...
  if (Object.hasOwn(updates, 'status'))
    assignments.push(
      `completed_at = CASE WHEN $${fields.indexOf('status') + 1}::varchar = 'completed' THEN COALESCE(completed_at, NOW()) ELSE NULL END`,
    )
  values.push(id)
  return pool.query(
...
export function archiveTask(id) {
  return pool.query(
    "UPDATE tasks SET status = 'archived', completed_at = NULL WHERE id = $1 RETURNING *",
    [id],
  )
}
