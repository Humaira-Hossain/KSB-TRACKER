...
const task = await request('/api/tasks', {
  ...
  body: JSON.stringify({
    ...
    rawNotes: 'Created by the API verification script.',
  }),
})
...
const updatedTask = await request(`/api/tasks/${task.id}`, {
  method: 'PATCH',
  body: JSON.stringify({
    rawNotes: 'Updated by the API verification script.',
...
const evidence = await request(`/api/tasks/${task.id}/evidence`, {
  method: 'POST',
  body: JSON.stringify({
    title: 'API verification evidence',
    situation: 'Testing the backend',
    task: 'Verify core routes',
    action: 'Called each endpoint',
...
if (!ksb || !acceptanceCriterion)
  throw new Error('The database must contain at least one KSB and one acceptance criterion.')
...
const acEvidenceByCode = await request(
  `/api/acceptance-criteria/${acceptanceCriterion.code}/evidence`,
)
...
if (!hasEvidence(taskDetail.evidence))
  throw new Error('Task detail did not include the created evidence.')
...
if (!hasEvidence(acEvidenceByCode.evidence))
  throw new Error('AC code evidence endpoint did not include linked evidence.')
if (!hasEvidence(ksbEvidenceByCode.evidence))
  throw new Error('KSB code evidence endpoint did not include linked evidence.')
...
console.table([
  {
    ...
    archivedStatus: archivedTask.status,
    evidenceId: evidence.id,
    ac: acceptanceCriterion.code,
    ksb: ksb.code,
