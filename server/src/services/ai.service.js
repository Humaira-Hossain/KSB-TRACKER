...
function requiredText(value, field) {
  if (typeof value !== 'string' || !value.trim())
    throw aiResponseError(`${field} must be a non-empty string.`)
  return value.trim()
}

function confidence(value) {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1)
    throw aiResponseError('confidence must be a number from 0 to 1.')
  return value
}
...
function validateSuggestions(items, field, catalogue) {
  ...
  return items.map((item) => {
    if (!item || typeof item !== 'object')
      throw aiResponseError(`${field} contains an invalid item.`)
    ...
    return {
      ...
      rationale: requiredText(item.rationale, `${field}.rationale`),
    }
  })
}
...
function validateGeneration(content, ksbs, acceptanceCriteria) {
  ...
  try {
    ...
    throw aiResponseError('model content was not JSON.')
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
    throw aiResponseError('model content must be a JSON object.')
  ...
  return {
    ...
    ksbSuggestions: validateSuggestions(
      parsed.ksbSuggestions,
      'ksbSuggestions',
      new Map(ksbs.map((item) => [item.code, item])),
    ),
    acSuggestions: validateSuggestions(
      parsed.acSuggestions,
      'acSuggestions',
      new Map(acceptanceCriteria.map((item) => [item.code, item])),
    ),
  }
}
...
export async function generateEvidenceFromNotes({ rawNotes, ksbs, acceptanceCriteria }) {
  ...
  try {
    ...
    throw aiResponseError('AI service did not return JSON.')
  }
  const content = payload?.choices?.[0]?.message?.content
  console.log('NVIDIA AI response content:', content)
  if (typeof content !== 'string')
    throw aiResponseError('AI service response did not contain message content.')
