...
import { listAcceptanceCriteria } from '../repositories/catalog.repository.js'
...
export async function getProgressInsights() {
  const [missingKsbs, criteria] = await Promise.all([
    pool.query(`SELECT k.code, k.description FROM ksbs k WHERE NOT EXISTS (
      SELECT 1 FROM evidence_ksbs ek JOIN evidence e ON e.id = ek.evidence_id
      JOIN tasks t ON t.id = e.task_id
      WHERE ek.ksb_id = k.id AND ek.review_status = 'accepted' AND t.status = 'completed'
    ) ORDER BY k.code`),
    listAcceptanceCriteria(),
  ])
  const missingKsbCodes = new Set(missingKsbs.rows.map((ksb) => ksb.code))
  return {
    missingKsbs: missingKsbs.rows,
    incompleteAcceptanceCriteria: criteria.rows
      .filter((criterion) => !criterion.is_complete)
      .map(({ code, description, level, ksb_codes: requiredKsbCodes }) => ({
        code,
        description,
        level,
        requiredKsbCodes,
        missingKsbCodes: requiredKsbCodes.filter((ksbCode) => missingKsbCodes.has(ksbCode)),
      })),
  }
}
