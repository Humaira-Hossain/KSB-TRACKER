...
import { evidenceStatuses, httpError, idFrom, requireChoice, requireText, reviewStatuses } from "../utils/request.js";
...
export async function createEvidence(request, response, next) {
  try {
    ...
    const result = await evidence.createEvidence(taskId, { ...request.body, title: requireText(request.body.title, "title"), status });
...
export async function updateEvidence(request, response, next) {
  try {
    ...
    if (Object.hasOwn(request.body, "status")) requireChoice(request.body.status, evidenceStatuses, "status");
    const updates = { ...request.body };
    for (const [apiField, databaseField] of Object.entries({ rawNotes: "raw_notes", aiGenerated: "ai_generated", userReviewed: "user_reviewed", reviewedAt: "reviewed_at" })) {
      if (Object.hasOwn(updates, apiField)) { updates[databaseField] = updates[apiField]; delete updates[apiField]; }
    }
...
export async function generateEvidence(request, response, next) {
  try {
    ...
    const generated = await generateEvidenceFromNotes({ rawNotes, ksbs: ksbs.rows, acceptanceCriteria: acceptanceCriteria.rows });
    const ksbByCode = new Map(ksbs.rows.map((ksb) => [ksb.code, ksb]));
    const acByCode = new Map(acceptanceCriteria.rows.map((ac) => [ac.code, ac]));
    const suggestions = {
      ksbs: generated.ksbSuggestions.map((suggestion) => ({ ...suggestion, id: ksbByCode.get(suggestion.code).id })),
      acceptanceCriteria: generated.acSuggestions.map((suggestion) => ({ ...suggestion, id: acByCode.get(suggestion.code).id })),
    };
    const savedEvidence = await evidence.saveGeneratedEvidence(evidenceId, generated, suggestions);
    response.json({
      evidence: savedEvidence,
      suggestions: {
        ksbs: suggestions.ksbs.map(({ id, ...suggestion }) => ({ ...suggestion, ksbId: id, reviewStatus: "suggested" })),
        acceptanceCriteria: suggestions.acceptanceCriteria.map(({ id, ...suggestion }) => ({ ...suggestion, acceptanceCriterionId: id, reviewStatus: "suggested" })),
      },
    });
  } catch (error) { next(error); }
}
...
export async function createKsbLink(request, response, next) {
  try {
    ...
    const reviewStatus = request.body.reviewStatus ?? (suggestedBy === "ai" ? "suggested" : "accepted");
    requireChoice(reviewStatus, reviewStatuses, "reviewStatus");
    if (suggestedBy === "ai" && reviewStatus !== "suggested") throw httpError(400, "AI links must remain suggestions until you review them.");
    response.status(201).json((await evidence.createKsbLink(evidenceId, ksbId, { aiConfidence: request.body.aiConfidence ?? null, suggestedBy, reviewStatus })).rows[0]);
  } catch (error) { next(error); }
}
...
export async function createAcceptanceCriterionLink(request, response, next) {
  try {
    ...
    if (!Number.isSafeInteger(acId) || acId < 1) throw httpError(400, "acceptanceCriterionId is required.");
    await requireEvidence(evidenceId);
    if (!(await catalog.getAcceptanceCriterion(acId)).rowCount) throw httpError(404, "Acceptance criterion not found.");
    const suggestedBy = request.body.suggestedBy ?? "user";
    if (!["ai", "user"].includes(suggestedBy)) throw httpError(400, "Invalid suggestedBy.");
    const reviewStatus = request.body.reviewStatus ?? (suggestedBy === "ai" ? "suggested" : "accepted");
    requireChoice(reviewStatus, reviewStatuses, "reviewStatus");
    const aiSuggestsComplete = request.body.aiSuggestsComplete ?? false;
    if (typeof aiSuggestsComplete !== "boolean") throw httpError(400, "aiSuggestsComplete must be true or false.");
    if ((suggestedBy === "ai" && reviewStatus !== "suggested") || (aiSuggestsComplete && suggestedBy !== "ai")) throw httpError(400, "Only an AI suggestion can recommend AC completion, and it must be reviewed by you.");
    response.status(201).json((await evidence.createAcceptanceCriterionLink(evidenceId, acId, { aiConfidence: request.body.aiConfidence ?? null, suggestedBy, reviewStatus, aiSuggestsComplete, aiCompletionRationale: request.body.aiCompletionRationale ?? null })).rows[0]);
  } catch (error) { next(error); }
}

export async function reviewKsbLink(request, response, next) {
  try {
    const result = await evidence.reviewKsbLink(idFrom(request), idFrom(request, "ksbId"), requireChoice(request.body.reviewStatus, reviewStatuses, "reviewStatus"));
...
export async function reviewAcceptanceCriterionLink(request, response, next) {
  try {
    const result = await evidence.reviewAcceptanceCriterionLink(idFrom(request), idFrom(request, "acId"), requireChoice(request.body.reviewStatus, reviewStatuses, "reviewStatus"));
