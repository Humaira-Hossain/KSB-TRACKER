...
export async function listTasks(_request, response, next) {
  try { response.json((await tasks.listTasks()).rows); } catch (error) { next(error); }
}
...
export async function createTask(request, response, next) {
  try {
    ...
    const result = await tasks.createTask({ title: requireText(request.body.title, "title"), rawNotes: requireText(request.body.rawNotes, "rawNotes"), status });
...
export async function updateTask(request, response, next) {
  try {
    ...
    if (Object.hasOwn(request.body, "status")) requireChoice(request.body.status, taskStatuses, "status");
    const updates = { ...request.body };
    if (Object.hasOwn(updates, "rawNotes")) { updates.raw_notes = updates.rawNotes; delete updates.rawNotes; }
