...
export async function listNotes(_request, response, next) {
  try { response.json((await notes.listNotes()).rows); } catch (error) { next(error); }
}

export async function createNote(request, response, next) {
  try {
    const result = await notes.createNote({ title: optionalTitle(request.body.title), content: requireText(request.body.content, "content") });
...
export async function updateNote(request, response, next) {
  try {
    ...
    if (Object.hasOwn(request.body, "content")) updates.content = requireText(request.body.content, "content");
