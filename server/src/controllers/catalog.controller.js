...
export async function listKsbs(_request, response, next) {
  try {
    response.json((await catalog.listKsbs()).rows)
  } catch (error) {
    next(error)
  }
}
...
export async function listAcceptanceCriteria(_request, response, next) {
  try {
    response.json((await catalog.listAcceptanceCriteria()).rows)
  } catch (error) {
    next(error)
  }
}
...
export async function getAcceptanceCriterionEvidenceByCode(request, response, next) {
  try {
    const [criterion, ksbs, evidence] = await catalog.getAcceptanceCriterionEvidenceByCode(
      codeFrom(request),
    )
    if (!criterion.rowCount) throw httpError(404, 'Acceptance criterion not found.')
    response.json({
      ...criterion.rows[0],
      requiredKsbCodes: ksbs.rows.map((ksb) => ksb.code),
